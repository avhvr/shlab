 ...
 *
 * <Venkata Harshavardhan Reddy Allu | avhvr>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int parseline(const char *cmdline, char **argv)
{
    ...
    while (*buf && isspace(*buf)) /* ignore leading spaces */
        buf++;
...
int builtin_cmd(char **argv)
{
    if (!strcmp(argv[0], "quit")) { /* quit */
        exit(0);
    } else if (!strcmp(argv[0], "&")) { /* singleton & */
        return 1;
    }
    else if (!strcmp(argv[0], "jobs")) { /* list jobs */
        listjobs(jobs);
        return 1;
    } else if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) {  /* bg | fg*/
        do_bgfg(argv);
        return 1;
    }
